...
import cn.com.xinli.android.mediarenderer.upnp.DefConnectionManagerService;
import cn.com.xinli.android.mediarenderer.upnp.DefMediaRenderer;
import cn.com.xinli.android.mediarenderer.utils.FixedAndroidHandler;
...
public class MediaRendererService extends Service {
	...
	public static String TAG = "MediaRendererService";
	...
	// upnp service
	private static final Logger log = Logger.getLogger(MediaRendererService.class.getName());
	private AndroidUpnpService upnpService;
    private UDN udn = UDN.uniqueSystemIdentifier("Xinli android media renderer");
    DefMediaRenderer mediaRenderer;
    private ServiceConnection serviceConnection = new ServiceConnection() {

        public void onServiceConnected(ComponentName className, IBinder service) {
            upnpService = (AndroidUpnpService) service;
            
//            DisplayHandler displayHandler = new ActivityDisplayHandler();
            
            /*mediaRenderer = new DefMediaRenderer();
            Intent intent = new Intent();
            intent.setClass(MediaRendererService.this, DefMediaRenderer.class);
            ...
            /*LocalService<AVTransportService> aVTransportService = getAVTransportService();

            // Register the device when this activity binds to the service for the first time
            if (aVTransportService == null) {
                try {
                    LocalDevice mediaRendererDevice = createDevice();

                    Toast.makeText(MediaRendererService.this, "Registering device", Toast.LENGTH_SHORT).show();
                    upnpService.getRegistry().addDevice(mediaRendererDevice);

                    aVTransportService = getAVTransportService();

                } catch (Exception ex) {
                    log.log(Level.SEVERE, "Creating MediaRenderer failed", ex);
                    Toast.makeText(MediaRendererService.this, "create_MediaRenderer_failed", Toast.LENGTH_SHORT).show();
                    return;
                }
            }
            else{
            	Log.d(TAG,"aVTransportService != null");
            }*/
        }
...
//        Logger.getLogger("org.fourthline.cling.support.avtransport").setLevel(Level.FINEST);
//
//        // UDP communication
...
//        );
        ...
        intent2.putExtra("numberOfPlayers", 1);
	...
	public void onDestroy() {
		...
		upnpService.getControlPoint().getRegistry().removeDevice(mediaRenderer.getDevice());
	...
	protected LocalService<AVTransportService> getAVTransportService() {
        if (upnpService == null)
            return null;
	...
	/*protected LocalDevice createDevice()
        ...
        DeviceDetails details =
                new DeviceDetails(
                        "MediaRenderer on " + android.os.Build.MODEL,
                        new ManufacturerDetails("Xinli"),
                        new ModelDetails("MediaRenderer", "A media renderer on android.", "v1")
                );

        LocalService<AVTransportService> serviceAvTransport =
                new AnnotationLocalServiceBinder().read(AVTransportService.class);

        serviceAvTransport.setManager(
                new DefaultServiceManager<AVTransportService>(serviceAvTransport, null) {
                    @Override
        ...
        LocalService<DefConnectionManagerService> serviceConnectionManager =
                new AnnotationLocalServiceBinder().read(DefConnectionManagerService.class);
        serviceConnectionManager.setManager(
                new DefaultServiceManager<DefConnectionManagerService>(
                		serviceConnectionManager,
                        DefConnectionManagerService.class
                )
        );
       
        LocalService[] services = {serviceAvTransport,serviceConnectionManager};
